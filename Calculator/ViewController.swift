...
class ViewController: UIViewController {
    ...
    @IBAction func calculateButtonClicked(_ sender: UIButton) {
        ...
        case 1107:// log10
            let logNumber = log10(Double(currentNumber))
            try! self.core.addStep(logNumber)
        ...
        if(self.tags == 1){ //x^y
            let powNumber = pow(x,currentNumber)
       //     try! self.core.addStep(100)
            try! self.core.addStep(*)
            try! self.core.addStep(0)
            try! self.core.addStep(+)
            try! self.core.addStep(powNumber)
            let result = self.core.calculate()!
            self.displayLabel.text = result.displayString
            // Reset the core
            self.core = Core<Double>()
        ...
        else if(self.tags == 2){ //nth root
            let powNumber = pow(x,(1/currentNumber))
            //     try! self.core.addStep(100)
            try! self.core.addStep(*)
            try! self.core.addStep(0)
            try! self.core.addStep(+)
